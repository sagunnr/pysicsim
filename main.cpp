...
namespace PhysicsSim {
    ...
    class Entity {
        ...
        EntityState state_ = EntityState::Dormant; // in-class init

        Vec2f position_;

        // In-class member initializers (Sonar fix)
        Vec2f momentum_{0, 0};
    ...
    public:
        Entity(const Vec2f& position,
            ...
            : properties_(std::move(props)), geometry_(std::move(geometry)), position_(position) {
            // Safe RNG usage: seeded only once for the whole app
    ...
    public:
        ...
        static void renderEntity(const Entity& e) {
            ...
            auto stateIndex = static_cast<size_t>(std::to_underlying(e.getState()));

            if (geom->type == GeometryType::Quadrilateral) {
                auto [min, max] = e.getBounds();
                std::cout << "Quadrilateral[" << states[stateIndex] << "]: bounds ["
                ...
                std::cout << "Spheroid[" << states[stateIndex] << "]: center ["
        ...
        static void renderInteraction(const Interaction& interaction) {
            ...
            auto age = std::chrono::duration<float>(
                std::chrono::high_resolution_clock::now() - interaction.detectionTime).count();
        ...
        static void renderKineticField(const Entity& e) {
            ...
            if (kineticEnergy > 0.1f) {
                std::cout << "Influence at [" << pos[0] << "," << pos[1]
                          << "]: KE=" << kineticEnergy
                          << ", momentum=[" << mom[0] << "," << mom[1] << "]\n";
            }
        }
    ...
    class Simulation {
        ...
        std::unique_ptr<PositionCorrection> posRes_ = std::make_unique<PositionCorrection>();    // In-class init
        std::unique_ptr<MomentumExchange> momRes_ = std::make_unique<MomentumExchange>();       // In-class init
    ...
    public:
        ...
        void runCycle() {
            ...
            for (auto& e : entities_) {
                if (e->getState() == EntityState::Static) continue;
                float m = e->getProperty("mass");
            ...
            for (size_t i = 0; i < n; ++i) {
                for (size_t j = i + 1; j < n; ++j) {
                    ...
                    if (!interaction.contact) continue;
        ...
        Entity* entity(size_t idx) {
            return idx < entities_.size() ? entities_[idx].get() : nullptr;
        }

        size_t interactionCount() const {
        ...
        void runMainLoop() {
            ...
            while (state_ != State::Terminate) {
                ...
                if (delta < 1.0f / 60 || state_ != State::Running) {
                    // Not time to run a frame or paused; check input & sleep
                    ...
                    continue;  // Early continue limits nesting
                }
                ...
                // Check for commands after simulation step
                if (std::cin.rdbuf()->in_avail() > 0) {
                    std::string cmd;
                    std::cin >> cmd;
                    handleCommand(cmd);
